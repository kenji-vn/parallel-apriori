...
namespace AprioriAlgorithm
{
    ...
    public class Apriori : IApriori
    {
        ...
        Output IApriori.ProcessTransaction2(double minSupport, double minConfidence, IEnumerable<string> items, DrillData transactions)
        {
            IList<Item> frequentItems = GetL1FrequentItemsP(minSupport, items, transactions);
            ItemsDictionary allFrequentItems = new ItemsDictionary();
            allFrequentItems.ConcatItems(frequentItems);
            IDictionary<string, double> candidates = new Dictionary<string, double>();
            double transactionsCount = transactions.Count();
            ...
            do
            {
                candidates = GenerateCandidatesP(frequentItems, transactions);
                frequentItems = GetFrequentItems(candidates, minSupport, transactionsCount);
                allFrequentItems.ConcatItems(frequentItems);
            }
            while (candidates.Count != 0);
        ...
        private List<Item> GetL1FrequentItemsP(double minSupport, IEnumerable<string> items, DrillData transactions)
        {
            var frequentItemsL1 = new List<Item>();
            double transactionsCount = transactions.Count();

            //query for 1 item set
            frequentItemsL1.Sort();
            return frequentItemsL1;
        }
        ...
        private double GetSupportP(string generatedCandidate, DrillData transactionsList)
        {
            double support = 0;
        ...
        private Dictionary<string, double> GenerateCandidatesP(IList<Item> frequentItems, DrillData transactions)
        {
            Dictionary<string, double> candidates = new Dictionary<string, double>();
            ...
            for (int i = 0; i < frequentItems.Count - 1; i++)
            {
                ...
                for (int j = i + 1; j < frequentItems.Count; j++)
                {
                    ...
                    if (generatedCandidate != string.Empty)
                    {
                        double support = GetSupportP(generatedCandidate, transactions);
                        candidates.Add(generatedCandidate, support);
                    }
                }
            }
